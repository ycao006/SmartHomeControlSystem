...
//		            			System.out.println(mCharacteristicRingBuffer.size());
		...
		public void onReadRemoteRssi(BluetoothGatt gatt, int rssi, int status) {
		 ...
		 //   if(status == BluetoothGatt.GATT_SUCCESS) {
		      //  final Intent intent = new Intent();
		        String intentAction = ACTION_RSSI_VALUE_READ;
		     //   intent.putExtra(BlunoLibrary.RSSII, rssi);
		    //    intent.setAction(ACTION_RSSI_VALUE_READ);              
		   //     sendBroadcast(intent);
		        broadcastUpdate(intentAction, rssi);
    ...
    private void broadcastUpdate(final String action, final int rssi) {
      ...
      //  intent.putExtra(BlunoLibrary.RSSII, rssi);
        String srssi;
        srssi=String.valueOf(rssi);
        intent.putExtra(BlunoLibrary.RSSII, srssi);
        intent.setAction(ACTION_RSSI_VALUE_READ);
        sendBroadcast(intent);
    }
...
//        }
        ...
        final BluetoothDevice device = mBluetoothAdapter.getRemoteDevice("D0:39:72:C5:35:2E");// 也可以直接设置UUID
        //final BluetoothDevice device = mBluetoothAdapter.getRemoteDevice("C4:BE:84:15:95:DD");// 也可以直接设置UUID
     ...
     /*   try {
        ...
        mConnectionState = STATE_CONNECTING;
    	//System.out.println("here"+address+mBluetoothDeviceAddress);
